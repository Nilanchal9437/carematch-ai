...
function calculateRatings(document: FacilityDocument): RatedFacilityDocument {
  ...
  const overallRating = Math.min(Math.max(parseFloat(document.overall_rating) || 0, 0), 5);
  const healthInspectionRating = Math.min(Math.max(parseFloat(document.health_inspection_rating) || 0, 0), 5);
  
  // Calculate occupancy rate with validation
  const residents = parseFloat(document.average_number_of_residents_per_day) || 0;
  ...
  const turnoverRate = Math.max(parseFloat(document.total_nursing_staff_turnover) || 0, 0);
  const numberOfFines = Math.max(parseInt(document.number_of_fines, 10) || 0, 0);
  const hasRecentInspection = document.most_recent_health_inspection_more_than_2_years_ago === "N";
  ...
  if (document.ownership_type?.toLowerCase().includes('for profit')) buyPoints += 1;
...
export async function GET(req: NextRequest) {
  try {
    ...
    const response = await axios.get('https://data.cms.gov/provider-data/api/1/datastore/query/4pq5-n9py/0');
    ...
    for (const home of nursingHomes) {
      try {
        ...
        console.log(`Processing nursing home with CCN: ${home.cms_certification_number_ccn}`);
        ...
        const existingId = existingHomesMap.get(home.cms_certification_number_ccn);
        ...
        if (existingId) {
          ...
          updatedCount++;
          console.log(`Added update operation for existing home: ${home.cms_certification_number_ccn}`);
        } else {
          // Insert new record
          operations.push({
            insertOne: {
          ...
          newCount++;
          console.log(`Added insert operation for new home: ${home.cms_certification_number_ccn}`);
        }
    ...
    if (operations.length > 0) {
      console.log(`Executing final batch of ${operations.length} operations...`);
      try { 
